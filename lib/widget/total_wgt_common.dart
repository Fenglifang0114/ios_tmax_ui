...
//选择PLU的
Widget showSelectPluWidget(BuildContext context, double width, double height,
    void Function(PluData) onPluSelected) {
  return Container(
    width: width,
    height: height,
    ...
    child: Autocomplete<PluData>(
      fieldViewBuilder: (BuildContext context,
        ...
        return TextField(
          ...
          decoration: InputDecoration(
            ...
            // 可以添加提示文本等其他装饰属性
            hintText: 'PLU', //localizedStrings.fSearchHint,
            hintStyle: TextStyle(
      ...
      optionsBuilder: (TextEditingValue textEditingValue) {
        ...
        resultSet.addAll(myPluInfoList.where((PluData data) {
        ...
        for (var item in resultSet) {
          ...
          if (item.recId != null && seenRecIds.add(item.recId!)) {
            newList.add(item);
            seenRecIds.add(item.recId!);
          }
        }

        return newList.toList()..sort((a, b) => a.plu!.compareTo(b.plu!));
      },
      onSelected: (PluData selection) {
        onPluSelected(selection);
      },
