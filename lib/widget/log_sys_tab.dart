...
class SysLogTabPage extends StatefulWidget {
  final String contentType;

  const SysLogTabPage({super.key, required this.contentType});

  @override
  State<SysLogTabPage> createState() => _SysLogTabPageState();
}

class _SysLogTabPageState extends State<SysLogTabPage> {
  ...
  late SyslogDataSource _dataSource;
  ...
  void initState() {
    ...
    _eventbus1 = eventBus.on<EventRespGetSysLogList>().listen((event) {
      if (mounted) {
        ...
        try {
          SysLogFormDb sysLogFormDb = sysLogFormDbFromJson(josnData);
    ...
    _eventbus2 = eventBus.on<EventRespDelSysLog>().listen((event) {
      if (mounted) {
        _currentPage = 1;
        _clearSelection();
        getCurrentPageLogs();
  ...
  void getCurrentPageLogs() {
    ...
    if (_sortField.isNotEmpty) {
      switch (_sortField) {
        ...
        case 'roleId':
          ...
          break;
        case 'funcName':
          sortFieldRename = 'func_name';
          break;
        case 'operationType':
          sortFieldRename = 'operation_type';
          break;
        case 'createTime':
    ...
    PublicFunctions.getSysLog(jsonStr);
  }

  void _loadSyslogData() {
    _initializeDataSource();
  }

  void _initializeDataSource() {
    _dataSource = SyslogDataSource(
      context: context,
      allSyslogData: _allSyslogs,
  ...
  void _updateMultipleSelection(Set<int> selectedIndexes) {
    ...
    final currentPageData = _allSyslogs.sublist(
      pageStartIndex,
      pageStartIndex + _pageSize > _allSyslogs.length
          ? _allSyslogs.length
              ...
              return SfDataGrid(
                ...
                onCellTap: (details) {
                  if (details.rowColumnIndex.rowIndex - 1 >= 0) {
                    if (_dataSource.allSyslogData.length <=
                        details.rowColumnIndex.rowIndex - 1) {
                      return;
                    }
                    final SysLog log = _dataSource
                        .allSyslogData[details.rowColumnIndex.rowIndex - 1];

                    showDialog(
                        context: context,
                        builder: (BuildContext context) {
                          return SyslogDetailDialog(log: log);
                        });
                  }
                },
              );
  ...
  List<GridColumn> _buildColumns(
    ...
    double columnWidth = (width - 40 - 80 - 200 - 120 - 200) / 5;
    if (columnWidth < 150) {
      columnWidth = 150;
    }
      ...
      child: Row(children: [
        ...
        showTextButton(context, btnHeight, (localizedStrings?.gBtnExport ?? "gBtnExport"),
          ...
          ReqExportLog exportLog = ReqExportLog(
            ...
            search: Search(
              searchOperator:
                  operatorCtl.text.isNotEmpty ? operatorCtl.text : null,
              module: null,
              roleId: roleIdCtl.text.isNotEmpty
                  ? int.tryParse(roleIdCtl.text)
                  : null,
              startTime:
                  dateStartCtl.text.isNotEmpty ? dateStartCtl.text : null,
...
class SyslogDataSource extends DataGridSource {
  final BuildContext context;
  final List<SysLog> allSyslogData;
  final ValueChanged<Set<int>> onMultipleSelectionChanged;

  List<SysLog> currentPageData = [];
  List<DataGridRow> _dataGridRows = [];
  Set<int> _selectedRowIndexes = {};

  SyslogDataSource({
    required this.context,
    required this.allSyslogData,
    required this.onMultipleSelectionChanged,
  }) {
    updateCurrentPage(1, 20);
  }

  void updateData(List<SysLog> newData) {
    allSyslogData.clear();
    allSyslogData.addAll(newData);
    updateCurrentPage(1, 20);
  }
  ...
  void updateCurrentPage(int currentPage, int pageSize) {
    ...
    currentPageData = allSyslogData.sublist(
      startIndex.clamp(0, allSyslogData.length),
      endIndex.clamp(0, allSyslogData.length),
    );
  ...
  void buildDataGridRows() {
    _dataGridRows = currentPageData.map<DataGridRow>((syslog) {
      return DataGridRow(cells: [
        ...
        DataGridCell<String>(
          columnName: 'roleId',
          value: getSysLogTrans(syslog.roleId.toString()),
        ),
        DataGridCell<String>(
          columnName: 'module',
          value: syslog.module == null ? '' : getSysLogTrans(syslog.module!),
        ),
        DataGridCell<String>(
          columnName: 'funcName',
          value:
              syslog.funcName == null ? '' : getSysLogTrans(syslog.funcName!),
        ),
        DataGridCell<String>(
          columnName: 'operationType',
          value: syslog.operationType == null
              ? ''
              : getSysLogTrans(syslog.operationType!),
        ),
        DataGridCell<String>(
          columnName: 'operation',
          value: syslog.operation,
        ),
        DataGridCell<String>(
          columnName: 'result',
          value: syslog.result == null ? '' : getSysLogTrans(syslog.result!),
        ),
        DataGridCell<String>(
  ...
  DataGridRowAdapter? buildRow(DataGridRow row) {
    ...
    final SysLog syslog = currentPageData[rowIndex];
    final colorScheme = Theme.of(context).colorScheme;
