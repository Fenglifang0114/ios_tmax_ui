...
class ReceiptDraggableFloating extends StatefulWidget {
  ...
  const ReceiptDraggableFloating({
    super.key,
  ...
  @override
  State<StatefulWidget> createState() => _ReceiptDraggableFloatingState();
}

class _ReceiptDraggableFloatingState extends State<ReceiptDraggableFloating> {
  ...
  void _setBoundary(_) {
    ...
    try {
      ...
      setState(() {
        ...
        _minOffset = const Offset(0, 0);
        _originOffset = const Offset(0, 0);
        myReceiptOffsetData.width = size.width;
        myReceiptOffsetData.height = size.height;
        //_maxOffset X/Y轴最大坐标
        if (myPrinter.printer == 'PT566') {
...
//20230330
  ...
  void _updatePosition(PointerMoveEvent pointerMoveEvent) {
    ...
    // if (myPrinter.printer == 'PT566') {
    //   newOffsetY = _offset.dy + pointerMoveEvent.delta.dy + myReceiptOffsetData.height;
    // }
  ...
  Widget build(BuildContext context) {
    // _key = GlobalKey();
    myReceiptOffsetData.x = (_offset.dx.toInt()).roundToDouble();
    ...
    return Positioned(
      ...
      child: Listener(
        onPointerMove: (PointerMoveEvent pointerMoveEvent) {
          ...
          double _currentX1 = _currentX + myReceiptOffsetData.width;
          double _currentY1 = _currentY + myReceiptOffsetData.height;
          cleanList();
        ...
        onPointerUp: (PointerUpEvent pointerUpEvent) {
          _offset = Offset(
              _offset.dx, nearestMultiple(_offset.dy, receiptLineHeight));
          myReceiptOffsetData.x = (_offset.dx.toInt()).roundToDouble();
          myReceiptOffsetData.y = ((_offset.dy).toInt()).roundToDouble();
          myReceiptOffsetData.key = widget.key!;
          // myReceiptOffsetDataList.offsetDataList.add(myReceiptOffsetData);
  ...
  double nearestMultiple(double value, double base) {
    // 将 value 除以 base 并四舍五入取整
    int multiple = (value / base).round();
    // 计算最近的整数值
    double result = (base * multiple) + 1;
    return result;
  }
  ...
  void checkPosition() {
    myPositions = Positions(
        myReceiptOffsetData.x,
        myReceiptOffsetData.y,
        myReceiptOffsetData.x + myReceiptOffsetData.width,
        myReceiptOffsetData.y + myReceiptOffsetData.height,
        widget.key!);
