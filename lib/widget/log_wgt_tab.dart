...
class ScaleWgtLogTabPage extends StatefulWidget {
  final String contentType;

  const ScaleWgtLogTabPage({super.key, required this.contentType});

  @override
  State<ScaleWgtLogTabPage> createState() => _ScaleWgtLogTabPageState();
}

class _ScaleWgtLogTabPageState extends State<ScaleWgtLogTabPage> {
  ...
  late WgtLogDataSource _dataSource;
  ...
  void initState() {
    ...
    _eventbus1 = eventBus.on<EventRespGetWgtLogList>().listen((event) {
      if (mounted) {
        ...
        try {
          WgtLogFormDb wgtLogFormDb = wgtLogFormDbFromJson(josnData);
    ...
    _eventbus2 = eventBus.on<EventRespDelWgtLog>().listen((event) {
      if (mounted) {
        _currentPage = 1;
        _clearSelection();
        getCurrentPageLogs();
  ...
  void getCurrentPageLogs() {
    ...
    PublicFunctions.getWgtLogList(jsonStr);
  }

  void _loadWgtLogData() {
    _initializeDataSource();
  }

  void _initializeDataSource() {
    _dataSource = WgtLogDataSource(
      context: context,
      allWgtLogData: _allWgtLogs,
  ...
  void _updateMultipleSelection(Set<int> selectedIndexes) {
    ...
    final currentPageData = _allWgtLogs.sublist(
      pageStartIndex,
      pageStartIndex + _pageSize > _allWgtLogs.length
          ? _allWgtLogs.length
              ...
              return SfDataGrid(
                ...
                onCellTap: (details) {
                  if (details.rowColumnIndex.rowIndex - 1 >= 0) {
                    if (_dataSource.allWgtLogData.length <=
                        details.rowColumnIndex.rowIndex - 1) {
                      return;
                    }
                    final ScaleWgtInfo log = _dataSource
                        .allWgtLogData[details.rowColumnIndex.rowIndex - 1];

                    showDialog(
                        context: context,
                        builder: (BuildContext context) {
                          return WgtLogDetailDialog(log: log);
                        });
                  }
                },
              );
  ...
  List<GridColumn> _buildColumns(
    ...
    double columnWidth = (width - 40 - 80 - 200 - 120 - 200) / 6;
    if (columnWidth < 150) {
      columnWidth = 150;
    }
...
class WgtLogDataSource extends DataGridSource {
  final BuildContext context;
  final List<ScaleWgtInfo> allWgtLogData;
  final ValueChanged<Set<int>> onMultipleSelectionChanged;

  List<ScaleWgtInfo> currentPageData = [];
  List<DataGridRow> _dataGridRows = [];
  Set<int> _selectedRowIndexes = {};

  WgtLogDataSource({
    required this.context,
    required this.allWgtLogData,
    required this.onMultipleSelectionChanged,
  }) {
    updateCurrentPage(1, 20);
  }

  void updateData(List<ScaleWgtInfo> newData) {
    allWgtLogData.clear();
    allWgtLogData.addAll(newData);
    updateCurrentPage(1, 20);
  }
  ...
  void updateCurrentPage(int currentPage, int pageSize) {
    ...
    currentPageData = allWgtLogData.sublist(
      startIndex.clamp(0, allWgtLogData.length),
      endIndex.clamp(0, allWgtLogData.length),
    );
  ...
  void buildDataGridRows() {
    _dataGridRows = currentPageData.map<DataGridRow>((syslog) {
      return DataGridRow(cells: [
        ...
        DataGridCell<String>(
          columnName: 'roleId',
          value: getWgtLogTrans(syslog.roleId.toString()),
        ),
        DataGridCell<String>(
          columnName: 'module',
          value: syslog.module == null ? '' : getWgtLogTrans(syslog.module!),
        ),
        DataGridCell<String>(
          columnName: 'weight',
          value: syslog.weight,
  ...
  DataGridRowAdapter? buildRow(DataGridRow row) {
    ...
    final ScaleWgtInfo syslog = currentPageData[rowIndex];
    final colorScheme = Theme.of(context).colorScheme;
  ...
  Widget _buildCellContent(DataGridCell dataGridCell, ScaleWgtInfo syslog,
      int rowIndex, bool isSelected) {
    final colorScheme = Theme.of(context).colorScheme;
    final textTheme = Theme.of(context).textTheme;
