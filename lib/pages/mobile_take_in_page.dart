...
class MobileTakeInPage extends StatefulWidget {
  final Function(String) onNavigate;
  final String lastRouteName;

  const MobileTakeInPage({
    super.key,
  ...
  @override
  State<MobileTakeInPage> createState() => _MobileTakeInPageState();
}

class _MobileTakeInPageState extends State<MobileTakeInPage> {
  int _selectedTab = 0; // 0: Weighing, 1: Record

  // Weight & Take-In Data Maps
  final Map<int, TakeInWeightInfo> _scaleWeightMap = {};
  final Map<int, double> _scaleLastWgtMap = {};
  final Map<int, double> _scaleTakeInWgtMap = {};
  final Map<int, bool> _scaleStartTakeInMap = {};

  // Independent Mode PLU data per scale
  final Map<int, PluData?> _independentPluMap = {};

  // Summary Mode PLU data & Summary Unit
  PluData? _summaryPluData;
  String _summaryUnit = 'kg';

  // Device drawer check states
  ...
  dynamic _eventBusUpdateSetting;
  ...
  void _initEventBusListeners() {
    _eventBusWeightData =
      ...
      if (req.scaleId != null && req.msgBody != null) {
        int id = req.scaleId!;
        double nowVal = double.tryParse(req.msgBody!.weightVal) ?? 0.0;

        setState(() {
          TakeInWeightInfo info = TakeInWeightInfo(
            weight: req.msgBody!.weightVal,
            unit: req.msgBody!.weightUnit,
            isZero: req.msgBody!.isZero,
            isNet: req.msgBody!.isNet,
            stable: req.msgBody!.isStable,
          );
          _scaleWeightMap[id] = info;

          bool isStarted = _scaleStartTakeInMap[id] ?? false;
          if (isStarted) {
            double lastVal = _scaleLastWgtMap[id] ?? 0.0;
            double incVal = nowVal - lastVal;
            _scaleTakeInWgtMap[id] = incVal > 0 ? incVal : 0.0;
          } else {
            _scaleTakeInWgtMap[id] = 0.0;
          }
        });
    ...
    _eventBusUpdateSetting =
        eventBus.on<EventUpdateSettingParam>().listen((event) {
      if (mounted) _loadUiConfFromDb();
    });
    ...
    _eventBusSettingParam = eventBus.on<EventSettingParam>().listen((event) {
      ...
      setState(() {
        mySettingParam = param;
        _isSummaryMode = (param.wgtMode == 1);
        _saveMode = (param.recMode == msgAuto) ? "Auto" : "Manual";
        if (param.stableTime.isNotEmpty) {
          _stableTime = param.stableTime;
    ...
    _eventBusPluList = eventBus.on<EventProductRecList>().listen((event) {
      if (!mounted) return;
      List<PluDataFromDb> pluList = event.obj;
      setState(() {
        myPluInfoList.clear();
        for (var item in pluList) {
          if (item.enabled == false) continue;
          PluData plu = PluData(
            item.recId,
            int.tryParse(item.plu ?? '0') ?? 0,
            int.tryParse(item.productCode ?? '0') ?? 0,
            int.tryParse(item.itemCode ?? '0') ?? 0,
            item.category,
            item.productName,
            int.tryParse(item.generalUnit ?? '0') ?? 0,
            int.tryParse(item.taxType ?? '0') ?? 0,
            double.tryParse(item.price ?? '0') ?? 0,
            double.tryParse(item.unitWeight ?? '0') ?? 0,
            double.tryParse(item.pretare ?? '0') ?? 0,
            double.tryParse(item.limitHigh ?? '0') ?? 0,
            double.tryParse(item.limitLow ?? '0') ?? 0,
          ...
          myPluInfoList.add(plu);
        }
      });
    });

    _eventBusSaveTrigger = eventBus.on<EventSaveTakeInOutWgt>().listen((event) {
      if (!mounted) return;
      for (var scale in myAllScalesList) {
        int id = scale.scaleId;
        if (_scaleStartTakeInMap[id] == true) {
  ...
  void _fetchSelScales() async {
    ...
    setState(() {
      for (var scale in myAllScalesList) {
        if (savedScales.contains(scale.scaleId)) {
          scale.isOnline = true;
          _drawerDeviceCheckedMap[scale.scaleId] = true;
        } else {
          _drawerDeviceCheckedMap[scale.scaleId] = false;
        }
      }
    });
  }
  ...
  void _saveUiConfToDb() {
    ...
    mySettingParam.id = int.tryParse(wgtTakeInMode) ?? 2;
    mySettingParam.scaleMode = int.tryParse(wgtTakeInMode) ?? 2;
  ...
  void _checkScaleAutoSave(int scaleId, TakeInWeightInfo info) {
    if (_saveMode != "Auto") return;
    int stableSecs = int.tryParse(_stableTime) ?? 2;
    if (stableSecs <= 0) stableSecs = 2;

    double incVal = _scaleTakeInWgtMap[scaleId] ?? 0.0;
      ...
      if (_scaleStableTimerMap[scaleId] == null) {
        ...
        _scaleStableTimerMap[scaleId] =
          ...
          if (duration >= stableSecs) {
            ...
            if (mounted &&
                info.stable &&
                (_scaleTakeInWgtMap[scaleId] ?? 0.0) > 0.001) {
  ...
  void _toggleStartTakeIn(int scaleId) {
    TakeInWeightInfo info = _scaleWeightMap[scaleId] ??
        TakeInWeightInfo(weight: '0.00', unit: 'kg', stable: false);
    double nowVal = double.tryParse(info.weight) ?? 0.0;

    bool isStarted = _scaleStartTakeInMap[scaleId] ?? false;

    if (!isStarted) {
      setState(() {
        _scaleStartTakeInMap[scaleId] = true;
        _scaleLastWgtMap[scaleId] = nowVal;
        _scaleTakeInWgtMap[scaleId] = 0.0;
      });
    } else {
      setState(() {
        _scaleStartTakeInMap[scaleId] = false;
  ...
  void _recordSummaryToDb() {
    ...
    if (myAllScalesList.isEmpty) {
      selScaleList.add(1);
      TakeInWeightInfo info = _scaleWeightMap[1] ??
          TakeInWeightInfo(weight: '0.00', unit: 'kg', stable: false);
      double incVal = _scaleTakeInWgtMap[1] ?? 0.0;
      double convertedW = convertUnit(
          incVal, info.unit.isNotEmpty ? info.unit : 'kg', _summaryUnit);
      ...
      for (var scale in myAllScalesList) {
        if (scale.isOnline) {
          ...
          TakeInWeightInfo info = _scaleWeightMap[id] ??
              TakeInWeightInfo(weight: '0.00', unit: 'kg', stable: false);
          double incVal = _scaleTakeInWgtMap[id] ?? 0.0;
          double convertedW = convertUnit(
              incVal, info.unit.isNotEmpty ? info.unit : 'kg', _summaryUnit);
          totalWgt += convertedW;
    ...
    final newAddRec = ReqAddWgtRec()
      ..mode = int.parse(wgtTakeInMode) // Always 2 for Increment Weighing
      ..detailRec = [];
    ...
    writelog("Add Increment Weighing Summary Payload: $jsonPayload");
  ...
  void _recordSingleScaleToDb(int scaleId) {
    ...
    String saveWgtStr = isStarted
        ? incVal.toStringAsFixed(2)
        : (double.tryParse(info.weight) ?? 0.0).toStringAsFixed(2);

    if ((double.tryParse(saveWgtStr) ?? 0) <= 0) {
      showTipInfo(
          localizedStrings?.gTipInvalidWeightData ?? "Invalid weight data",
          context);
      return;
    }

    PluData tempPlu = _independentPluMap[scaleId] ??
        PluData(null, null, null, null, null, null, null, null, null, null,
            null, null, null, null, null, null, null, null, '', '');

    final newAddRec = ReqAddWgtRec()
      ..mode = int.parse(wgtTakeInMode)
      ..detailRec = [];
    ...
    final headerCommon = Header(
      id: '1',
      scaleModel: scaleObj?.scaleModel ?? '',
      scaleSn: scaleObj?.scaleSn ?? '',
      scaleName: scaleObj?.scaleName ?? 'Scale$scaleId',
      plu: (tempPlu.plu == null) ? "" : tempPlu.plu.toString(),
      productCode:
          (tempPlu.productCode == null) ? "" : tempPlu.productCode.toString(),
      itemCode: (tempPlu.itemCode == null) ? "" : tempPlu.itemCode.toString(),
    ...
    String jsonPayload = reqAddWgtRecToJson(newAddRec);
    writelog("Add Increment Weighing Single Scale Payload: $jsonPayload");
    PublicFunctions.addSummaryData(jsonPayload);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
          ...
          children: [
            ...
            InkWell(
              ...
              child: Container(
                ...
                child:
                    const Icon(Icons.scale, color: Color(0xFF1E293B), size: 20),
              ),
            ),
          ],
        ),
      ...
      body: Column(
        children: [
          // Segmented Navigation Header (Weighing | Record)
          Container(
            color: Colors.white,
  ...
  Widget _buildWeighingTabContent() {
    return SingleChildScrollView(
      child: Column(
        children: [
          // Sub Header Row: Current Mode Name & Settings Button ⚙️
          Padding(
          ...
          // Summary Header Card when in Summary Mode
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          // 3. Sub-Weight Display Row (Increment Weight)
          Padding(
          ...
          // 4. Action Buttons Row (Tare, Zero, Start/Stop, Save)
          Padding(
              ...
              children: [
                ...
                _buildActionButton(
                  ...
                  color: isStarted
                      ? const Color(0xFFEF4444)
                      : const Color(0xFF004884),
                  onPressed: isOnline ? () => _toggleStartTakeIn(id) : null,
                ),
                if (!_isSummaryMode) ...[
                  const SizedBox(width: 10),
                  _buildActionButton(
                  ...
                  Expanded(
                    child: filteredList.isEmpty
                        ...
                        : GridView.builder(
                            ...
                            itemBuilder: (context, index) {
                              PluData item = filteredList[index];
                              return InkWell(
                                onTap: () {
                                  setState(() {
                                    if (scaleId == null) {
                                      _summaryPluData = item;
                                    } else {
                                      _independentPluMap[scaleId] = item;
                                    }
                                  });
                                  Navigator.pop(context);
                                },
