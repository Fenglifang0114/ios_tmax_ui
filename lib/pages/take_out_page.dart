...
import 'package:t_max/widget/wgt_value_take_out_mode.dart';
...
// 使用 ValueNotifier 来存储总重量和稳定状态
  ...
  setSelScaleInApp() async {
    await AppSelScalesManager.setIntList(AppNames.taou, mySelScaleIdList);
  }

  getSelScaleInApp() async {
    List<int> savedScales = await AppSelScalesManager.getIntList(AppNames.taou);
  ...
  bool getTotalWgtStable() {
    ...
    for (final entry in scaleWeightMapCommon.entries) {
  ...
  double calculateTotalWeight() {
    ...
    for (var entry in scaleWeightMapCommon.entries) {
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                if (!_scaleWidgetCache.containsKey(scaleId)) {
                  _scaleWidgetCache[scaleId] = ScaleWgtTakeOutWidget(
                          ...
                          return Tooltip(
                            ...
                            child: IconButton(
                              ...
                              icon: getSvgIcon(
                                ...
                                28,
                                28,
                                totalWgtStable && totalWeightNotifier.value > 0
...
void sendDataToDb(
  ...
  eventBus.fire(EventSaveTakeInOutWgt(''));
}
